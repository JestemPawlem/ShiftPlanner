...
EmployeeRegistryListModel::EmployeeRegistryListModel(IEmployeeRegistryManager& manager, QObject* parent) :
...
int EmployeeRegistryListModel::rowCount(const QModelIndex& parent) const
{
	return parent.isValid() ? 0 : static_cast<int>(_templates.size());
}

QVariant EmployeeRegistryListModel::data(const QModelIndex& index, int role) const
{
...
Qt::ItemFlags EmployeeRegistryListModel::flags(const QModelIndex& index) const
{
	if (!index.isValid())
...
bool EmployeeRegistryListModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
...
bool EmployeeRegistryListModel::removeRows(int row, int count, const QModelIndex& parent)
{
...
void EmployeeRegistryListModel::refresh()
{
	beginResetModel();
	_templates = _manager.registriesNames();
	endResetModel();
